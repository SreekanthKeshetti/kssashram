...
//       query.admissionStatus = { $nin: ["Draft", "In Review", "Rejected"] };
//     } else if (role === "ksa_manager") {
//       // FIX: Use $in to catch both the correct spelling and the typo in legacy imported data
//       query.branch = { $in: ["Karunya Sindhu", "Karunya Sindu"] };
//       query.admissionStatus = { $nin: ["Draft", "In Review", "Rejected"] };
//     } else if (
//       !["admin", "president", "secretary", "treasurer"].includes(role)
//     ) {
...
//     if (req.body.healthRecords) student.healthRecords = req.body.healthRecords;
//     if (req.body.activities) student.activities = req.body.activities;
//     if (req.body.newActivityEntry)
//       student.activities.push(req.body.newActivityEntry);
//     if (req.body.sponsor !== undefined) student.sponsor = req.body.sponsor;
...
//     if (req.body.alumniDetails) student.alumniDetails = req.body.alumniDetails;
...
//     student.leaves.push({ startDate, endDate, reason, status: "On Leave" });
...
//           )
//             normalizedBranch = "Karunya Sindhu"; // <--- FIXED TYPO HERE
...
//   downloadBlankForm,
// };
//  above is in  after demo3 before cwc and after adhar and pan
const fs = require("fs");
const path = require("path");
const csv = require("csv-parser");
const Student = require("../models/Student");
...
const createStudent = async (req, res) => {
  try {
    ...
    const student = await Student.create({
      ...
      createdBy: req.user._id,
      admissionNumber,
      caseNumber, // Still saved as caseNumber in DB
      studentType,
      alternateContact,
      currentClass,
      aadhaarNumber,
  ...
  try {
    fs.createReadStream(filePath)
      ...
      .on("data", (row) => {
        ...
        const adminNo = getValue(["CCI_Admin", "Admin_No"]);
        // --- UPDATED ALIAS FOR CWC NUMBER ---
        const caseNo = getValue([
          "Case_Profile",
          "CaseProfile",
          "CWC",
          "CWC_Allotted_No",
        ...
        if (rawName) {
          results.push({
            admissionNumber: adminNo,
            caseNumber: caseNo, // Stored as caseNumber in DB, displayed as CWC
            firstName: fName,
            lastName: lName,
            currentClass: cls,
            contactNumber: mobile || "0000000000",
