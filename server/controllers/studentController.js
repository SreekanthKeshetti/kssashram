...
const deleteDocument = async (req, res) => {
  try {
    ...
    // (In Phase 2, you can add Cloudinary's destroy() API here to actually delete it from their servers to save space)

    res.json({ message: "File link removed", documents: student.documents });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
