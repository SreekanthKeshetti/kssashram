...
// --- THIS IS THE NEW IMPORT WE NEED ---
  ...
  cron.schedule("0 10 * * *", async () => {
    ...
    try {
      ...
      const donationsToRemind = await Donation.find({
        ...
        donorEmail: { $exists: true, $ne: "" }, 
        $or:[
          { nextReminderDate: { $gte: startOfDay(target30), $lte: endOfDay(target30) } },
          { nextReminderDate: { $gte: startOfDay(target7), $lte: endOfDay(target7) } },
        ],
      });
      ...
      for (const don of donationsToRemind) {
        const daysLeft = Math.ceil((new Date(don.nextReminderDate) - today) / (1000 * 60 * 60 * 24));
        ...
        // --- 2. SEND WHATSAPP ---
        if (don.donorPhone && don.donorPhone !== "0000000000") {
          await sendWhatsAppMessage(don.donorPhone, "recurring_reminder", {
            name: don.donorName,
            scheme: don.scheme,
            daysLeft: daysLeft,
          });
  ...
  cron.schedule("30 9 * * *", async () => {
    ...
    try {
      ...
      const getEventQuery = (daysAhead) => {
        ...
        const month = targetDate.getMonth() + 1;
        const fullStart = new Date(new Date(targetDate).setHours(0, 0, 0, 0));
        const fullEnd = new Date(new Date(targetDate).setHours(23, 59, 59, 999));
        ...
        return {
          ...
          query: {
            ...
            programDate: { $exists: true, $ne: null },
            $or:[
              { isRecurring: false, programDate: { $gte: fullStart, $lte: fullEnd } },
              {
                isRecurring: true,
                $expr: {
                  $and: [
      ...
      for (const job of jobs) {
        ...
        for (const don of upcomingEvents) {
          const eventDateStr = job.targetDateObj.toLocaleDateString("en-IN", { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
          const occasionText = don.occasion ? `on the occasion of ${don.occasion}` : "";
          const nameText = don.inNameOf ? `in the name of ${don.inNameOf}` : "";
          ...
          if (don.donorEmail) {
            ...
            await transporter.sendMail(mailOptions);
            console.log(`📧 [EMAIL] Sent Event Invite to ${don.donorName} (${job.daysAhead} days prior)`);
          }

          // --- 2. SEND WHATSAPP ---
          if (don.donorPhone && don.donorPhone !== "0000000000") {
            await sendWhatsAppMessage(don.donorPhone, "event_invitation", {
              name: don.donorName,
              scheme: don.scheme,
              date: eventDateStr,
            });
